...
def get_valid_performance(data_mode,DATA, MASK, in_parser, out_itr, eval_time=None, MAX_VALUE = -99, OUT_ITERATION=5, seed=1234, networkseed=1234):
    
    tf.set_random_seed(networkseed)
    ...
    if (data_mode=="MY-SYNTHETHIC" or data_mode == 'MY-SYNTHETIC-E1') :
        ...
        if(data_mode[:4]=='SEER'):
            setSizes=[60898,24361,36539] # Train, Validation, Test
    ...
    for itr in range(iteration):
        ...
        if stop_flag > 5: #for faster early stopping
            ...
            if (itr+1)%1000 == 0:
                ...
                #break validation data if too large
                packSize=5000                
                subNum=pred.shape[0]
                tmp_valid_list=list()
                if(subNum>5000):
                    totalPacks=subNum//packSize
                    for i in range(totalPacks+1):
                        for t, t_time in enumerate(eval_time):
                            ...
                            if eval_horizon >= num_Category:
                                ...
                                risk = np.sum(pred[i*packSize:min((i+1)*packSize,subNum),:,:(eval_horizon+1)], axis=2) #risk score until eval_time
                                for k in range(num_Event):
                                    # va_result1[k, t] = c_index(risk[:,k], va_time, (va_label[:,0] == k+1).astype(int), eval_horizon) #-1 for no event (not comparable)
                                    va_result1[k, t] = weighted_c_index(tr_time, (tr_label[:,0] == k+1).astype(int), risk[:,k], va_time[i*packSize:min((i+1)*packSize,subNum)], (va_label[i*packSize:min((i+1)*packSize,subNum),0] == k+1).astype(int), eval_horizon)
                        tmp_valid = np.mean(va_result1)
                        tmp_valid_list.append(tmp_valid)
                else:
                    for t, t_time in enumerate(eval_time):
                        eval_horizon = int(t_time)
                        if eval_horizon >= num_Category:
                    ...
                    tmp_valid = np.mean(va_result1)
                    tmp_valid_list.append(tmp_valid)
                tmp_valid=np.mean(np.asarray(tmp_valid_list))
                if tmp_valid >  max_valid:
                    ...
                    stop_flag += 1
            end=machineTime.time()
            elapsed=end-start
            allElapsed.append(elapsed)
    avgTime=np.mean(np.asarray(allElapsed))
    return max_valid,avgTime,itCount
